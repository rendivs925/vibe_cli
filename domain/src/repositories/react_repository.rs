...
use crate::entities::react::{ReactSession, ReactStep, ProposedCommand};
use std::error::Error;
...
pub trait ReactCommandRepository: Send + Sync {
    ...
    async fn get_commands_by_step(&self, step_id: &str) -> Result<Vec<ProposedCommand>, Box<dyn Error>>;
    async fn get_pending_commands(&self, step_id: &str) -> Result<Vec<ProposedCommand>, Box<dyn Error>>;
}