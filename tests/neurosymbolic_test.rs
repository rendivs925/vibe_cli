...
async fn test_neurosymbolic_integration() {
    ...
    let neuroservice =
        vibe_cli::application::services::neurosymbolic_service::NeurosymbolicService::new(
    ...
    assert!(
        result.is_ok(),
        "Neurosymbolic command generation should succeed"
    );
    ...
    if let Ok(plan) = result {
        ...
        println!(
            "❌ Neurosymbolic Integration Test Failed: {:?}",
            result.err()
        );
    }
}
