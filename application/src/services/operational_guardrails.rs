...
impl OperationalGuardrails {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        if self.groundedness {
            rules.push(
                "**Groundedness**: Base all answers strictly on the CONTEXT_VAULT. Do NOT hallucinate."
                    .to_string(),
            );
        }
        ...
        if self.recency_priority {
            let ref_id = self
                ...
                .unwrap_or("REF-02");
            rules.push(format!(
                "**Recency Priority**: Most recent [{}] (latest_output) overrides all prior references.",
                ref_id
            ));
        }
        if self.delta_only {
