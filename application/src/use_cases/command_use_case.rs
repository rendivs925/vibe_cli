...
impl CommandUseCase {
        ...
        let neurosymbolic_response = match self.neurosymbolic_service.process_query(input).await {
            Ok(response) => {
                ...
                if !response.ranked_solutions.is_empty() {
                    println!("🏆 Top Solution: {}", response.ranked_solutions[0].description);
                    println!("🧠 Neural Score: {:.1}", response.ranked_solutions[0].neural_score);
                    println!("⚙️ Symbolic Score: {:.1}", response.ranked_solutions[0].symbolic_score);
                    ...
                    let command = domain::entities::command::Command::new(
                        ...
                        response.ranked_solutions[0].solution.command_sequence.first().unwrap_or(&"").clone(),
                        vec![],
                    ...
                    CommandExecutionPlan::cached(
                        domain::entities::command::Command::new(
                            "fallback".to_string(),
            ...
            Err(e) => {
                ...
                Err(AppError::domain(format!("Neurosymbolic service error: {}", e)))
            }
        };
    ...
    pub async fn generate_multi_step(&self, input: &str) -> Result<MultiStepExecutionPlan, AppError> {
        ...
        if !plan_result.is_safe_to_execute() {
            return Err(AppError::safety("Multi-step plan failed safety validation".to_string()));
        }
