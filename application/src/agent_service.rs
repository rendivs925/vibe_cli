...
/// Coordinator for managing agent execution lifecycle and avoiding closure lifetime issues
...
impl ExecutionCoordinator {
    ...
    /// Internal method with owned context to avoid lifetime issues
    async fn execute_iteration_owned(
        &self,
        goal: String,
        state: AgentExecutionState,
        request: &AgentRequest,
        context: Arc<AgentExecutionContext>,
    ) -> anyhow::Result<AgentIterationResult> {
        // Initialize context for this iteration
        let agent_context = self.initialize_context(request, &context).await?;

        // Generate reasoning plan (adapt based on iteration state)
        let reasoning_steps = self.generate_reasoning(&goal, &agent_context, &context).await?;

        // Determine if tools are needed
        let tool_calls = if self.needs_tools(&goal, &reasoning_steps) {
            self.plan_tool_calls(&goal, &reasoning_steps, &agent_context, &context).await?
        } else {
            Vec::new()
        };

        // Limit tools per iteration (enforced by controller, but double-check)
        let max_tools = self.controller.max_tools_per_iteration() as usize;
        let tool_calls = if tool_calls.len() > max_tools {
            tool_calls[..max_tools].to_vec()
        } else {
            tool_calls
        };
        ...
        let tool_results = if !tool_calls.is_empty() {
            self.execute_tool_calls(&tool_calls, &mut agent_context.clone(), &context).await?
        } else {
            Vec::new()
        };
    ...
    // Helper methods that mirror the agent service functionality but with owned context
    ...
    async fn generate_reasoning(
        ...
        let response = context.ollama_client.generate_response_with_system(&prompt, &system_prompt)
            .await
    ...
    async fn plan_tool_calls(
        ...
        let tool_descriptions: Vec<String> = agent_context.available_tools
            .iter()
    ...
    async fn execute_tool_calls(
        ...
        for tool_call in tool_calls {
            let result = match tool_call.name.as_str() {
                "rag_query" => self.execute_rag_query(tool_call, agent_context, context).await,
                "file_read" => self.execute_file_read(tool_call, agent_context, context).await,
                "code_analysis" => self.execute_code_analysis(tool_call, agent_context, context).await,
                _ => Ok(ToolResult {
    ...
    async fn execute_file_read(
        ...
        if let Err(e) = context.sandbox.test_command("cat", &vec![file_path.to_string()]) {
    ...
    async fn generate_final_response(
        ...
        context.ollama_client.generate_response_with_system(&prompt, system_prompt)
            .await
...
impl AgentService {
    ...
    pub async fn process_request(&self, request: &AgentRequest) -> Result<AgentResponse> {
        // For now, fall back to single iteration until lifetime issues are fully resolved
        // TODO: Implement full multi-iteration execution once lifetime issues are resolved
        ...
        let tool_calls: Vec<ToolCall> = iteration_result.tool_calls.iter().enumerate().map(|(i, tool_str)| {
            ToolCall {
                id: format!("tool_{}", i),
                name: tool_str.clone(),
        ...
        let agent_result = infrastructure::agent_control::AgentResult {
        ...
        // Convert tool calls to proper format (simplified - in real implementation would parse from execution history)
        let tool_calls = Vec::new(); // TODO: Extract actual tool calls from execution history

        // Convert to response format
        Ok(AgentResponse {
            reasoning: vec!["Multi-iteration bounded execution completed".to_string()], // TODO: Extract reasoning from execution history
            tool_calls,
